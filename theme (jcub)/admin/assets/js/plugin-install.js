jQuery(document).ready(function($) {
    'use strict';
    var this_obj = jcub_companion_install;

    $(document).on('click', '.jcubweb-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message jcubweb-plugin-install');
                button.addClass('button-primary jcubweb-plugin-activate');
    ...
    $(document).on('click', '.jcubweb-plugin-activate', function(event) {
        event.preventDefault();
        var button = $(this);
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'jcub_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.jcubweb-customizer-notification-dismiss', function(event) {
        event.preventDefault();
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'jcub_hide_customizer_notice';
            sdata.jcub_plugin_slug = slug;
        }else{
            sdata.action = 'jcub_hide_customizer_companion_notice';
        }
